import Image from "next/image";
import Link from "next/link";
import type { Metadata } from "next";

export const metadata: Metadata = {
    title: "RCS LOGISTICS CAMBODIA LTD",
    description: "LOGISTICS FOR A GLOBAL ECONOMY",
};

export default function Page() {
    return (
        <>
            <section className="page-header" style={{ backgroundImage: "url('/images/img(7).jpg')" }}>
                <div className="container page-header-content">
                    <h1 className="page-title">Warehouse & Distribution</h1>
                    <div className="breadcrumb">
                        <Link href="/" >Home</Link>
                        <span className="breadcrumb-icon"><i className="fa-solid fa-angle-right"></i></span>
                        <Link href="/services" >Services</Link>
                        <span className="breadcrumb-icon"><i className="fa-solid fa-angle-right"></i></span>
                        <span>Warehouse & Distribution</span>
                    </div>
                </div>
            </section>


            <section className="section-padding">
                <div className="container">
                    <div className="row">
                        <h2 className="visually-hidden">Page Content</h2>

                        <div className="col-lg-4 mb-5 mb-lg-0 order-2 order-lg-1">


                            <div className="sidebar-widget">
                                <h2 className="widget-title">All Services</h2>
                                <ul className="cat-list service-list-widget">
                                    <li><Link href="/service-air-freight" >Air Freight <i className="fa-solid fa-arrow-right"></i></Link></li>
                                    <li><Link href="/service-sea-freight" >Sea Freight <i className="fa-solid fa-arrow-right"></i></Link></li>
                                    <li><Link href="/service-warehouse" className="active">Warehouse & Distribution<i className="fa-solid fa-arrow-right"></i></Link></li>
                                    <li><Link href="/service-buyer-consolidation">Bayer Consolidation<i className="fa-solid fa-arrow-right"></i></Link></li>
                                    <li><Link href="/service-project-cargo" >Project Cargo<i className="fa-solid fa-arrow-right"></i></Link></li>
                                    <li><Link href="/service-custom-brokerage">Customs Brokerage<i className="fa-solid fa-arrow-right"></i></Link></li>
                                </ul>
                            </div>


                            <div className="sidebar-widget">
                                <h2 className="widget-title">Downloads</h2>
                                <div className="brochure-box">
                                    <a href="#" className="brochure-btn">
                                        <span className="icon"><i className="fa-regular fa-file-pdf"></i></span>
                                        <span className="text">
                                            <span className="title">Service Brochure</span>
                                            <span className="size">PDF, 2.5MB</span>
                                        </span>
                                        <span className="download"><i className="fa-solid fa-download"></i></span>
                                    </a>
                                    <a href="#" className="brochure-btn mt-3">
                                        <span className="icon"><i className="fa-regular fa-file-word"></i></span>
                                        <span className="text">
                                            <span className="title">Company Profile</span>
                                            <span className="size">DOC, 1.2MB</span>
                                        </span>
                                        <span className="download"><i className="fa-solid fa-download"></i></span>
                                    </a>
                                </div>
                            </div>


                            <div className="sidebar-widget p-0 overflow-hidden text-center position-relative">
                                <Image src="/images/img(7).jpg" alt="Ad" className="img-fluid h-100 w-100" unoptimized width={1920} height={1080} sizes="100vw" />
                                <div className="position-absolute top-0 start-0 w-100 h-100 d-flex flex-column justify-content-center align-items-center bg-dark bg-opacity-75 text-white p-4">

                                    <h3>Need Help?</h3>
                                    <p>Call us for any logistics questions.</p>
                                    <h3 className="mb-4 text-white">Tel: +(855) 23 883 886/887</h3>
                                    <a href="#" className="btn btn-primary" data-hover="Contact Us">Contact Us</a>
                                </div>
                            </div>

                        </div>


                        <div className="col-lg-8 ps-lg-5 order-1 order-lg-2">
                            <div className="service-detail-image mb-5">
                                <Image src="/images/img(7).jpg" alt="Land Transport" className="img-fluid h-100 rounded" unoptimized width={1920} height={1080} sizes="100vw" />
                            </div>

                            <div className="service-detail-content">
                                <h2 className="mb-4">Warehouse & Distribution Services</h2>
                                <p>RCS Logistics Cambodia operates a 4,800 square meter warehouse facility strategically located in Phnom Penh, designed to support a wide range of logistics and supply chain solutions for both domestic and international customers.</p>
                                <h3 style={{ fontWeight: 700, fontSize: '1.25rem', marginBottom: '0.5rem', color: '#000' }}>
                                    Key Warehouse Features
                                </h3>
                                <ul style={{ listStyleType: 'disc', paddingLeft: '20px', marginBottom: '1.5rem', lineHeight: 1.6, fontSize: '1.05rem', fontWeight: 500 }}>
                                    <li>Vendor & buyer consolidation (CFS)</li>
                                    <li>General warehousing</li>
                                    <li>Distribution center (DC) operations</li>
                                    <li>Import/export cargo handling</li>
                                    <li>Export Assortment operations</li>
                                    <li>Short-term and long-term storage</li>
                                    <li>Container handling and trucking coordination</li>
                                    <li>Customs clearance support</li>
                                </ul>

                                <h3 style={{ fontWeight: 700, fontSize: '1.25rem', marginBottom: '0.5rem', color: '#000' }}>
                                    Security & Compliance
                                </h3>
                                <ul style={{ listStyleType: 'disc', paddingLeft: '20px', lineHeight: 1.6, fontSize: '1.05rem', fontWeight: 500 }}>
                                    <li>24/7 security monitoring/CCTV</li>
                                    <li>Controlled warehouse access</li>
                                    <li>Clean and organized storage environment</li>
                                    <li>Compliance with customer handling requirements</li>
                                </ul>
                                <div className="row my-5">
                                    <div className="col-md-6 mb-4 mb-md-0">
                                        <Image src="/images/img(4).jpg" alt="Feature" className="img-fluid rounded img-vertical-height" unoptimized width={1920} height={1080} sizes="100vw" />
                                    </div>
                                    <div className="col-md-6">
                                        <h3 className="mb-3">Why Choose Warehouse & Distribution</h3>
                                        <p>Overland transport is the most flexible shipping method, offering direct access to any location accessible by road.</p>
                                        <ul className="check-list dark-text mt-4">
                                            <li><i className="fa-solid fa-check"></i><span>Door-to-Door Service</span></li>
                                            <li><i className="fa-solid fa-check"></i><span>Flexible Routes & Schedules</span></li>
                                            <li><i className="fa-solid fa-check"></i><span>Cost-Effective for Regional</span></li>
                                            <li><i className="fa-solid fa-check"></i><span>Real-Time GPS Tracking</span></li>
                                        </ul>
                                    </div>
                                </div>

                                <h3 className="mb-4">Address of warehouse</h3>
                                <ul style={{ listStyleType: 'disc', paddingLeft: '20px', marginBottom: '1.5rem', lineHeight: 1.6, fontSize: '1.05rem', fontWeight: 500 }}>
                                    <li> Kilometer 20th, Broveok Porng Village, Road 41, Teok La’ork Commune, Korng Pisey District, Kompong Speu Province, Cambodia.</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
        </>
    );
}
